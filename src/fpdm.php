...
if (!class_exists(...$__tmp)) {
    ...
    define('FPDM_CACHE', __DIR__.'/export/cache/'); //cache directory for fdf temporary files needed by pdftk.
...
//					$this->Error("set_field_value failed as invalid valuetype $type for object $object_id");
        ...
        **/
        ...
        //------------------------------
            ...
            return (int)$chunks[1][1] -4; //-4 , relative to end of xref
        }
        ...
        //------------------------------
            ...
            $offset= (int)$chars[1][1] -strlen($object_header)+strlen($previous_object_footer)+2;
            return $offset;
        }
        ...
        *
        *@param string $name a string matching one of the supported default filters (marked with +)		*
        *Without parameters:
        ...
        *@return mixed the wished filter class to access the stream
        **/
         ...
         * 
         * @access public
         * @param string $stream_content the string content of the stream
         * @return boolean 
         */
                                    ...
                                    */
                                    ...
                                    if(($type=='')||($subtype=='')||($name=="")) {
                                        ...
                                        if(preg_match("/^\/(V|DV|TU)\s+([\<\(])/",$CurLine,$def)) {
                                            ...
                                            $maxLen=$values[1];
                                            $object["constraints"]["maxlen"]= (int)$maxLen;
                                        } else
                    ...
                    switch($xref_table) {
                        case 2:
                            if(preg_match("/^(\d+) (\d+)/",$CurLine,$match)) {
                                $refs_count= (int)$match[2];//xref_table length+1 (includes this line)
                                    ...
                                    switch($trailer_table) {
                                        ...
                                        case -2://startxref's value
                                            if(preg_match("/^(\d+)/",$CurLine,$match)) {
                                                $lines['$_XREF_$']["infos"]["start"]["value"]= (int)$match[1];
